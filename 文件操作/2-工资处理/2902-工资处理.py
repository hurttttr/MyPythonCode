...
for index,values in enumerate(sheet.rows,1):
    if index>1:
        ...
        sheet1.cell(row=index,column=8).value = round(lst[1]*0.7*(1+(lst[2]-40)*0.015),2)
        sy_dic[(lst[4],lst[5])]=sy_dic.get((lst[4],lst[5]),0)+round(lst[1]*0.7*(1+(lst[2]-40)*0.015),2)
...
for index,values in enumerate(sheet.rows,1):
    if index>1:
        ...
        ll_dic[(lst[4],lst[5])]=ll_dic.get((lst[4],lst[5]),0)+round(ans,2)
...
for index,values in enumerate(sheet1.rows,1):
    if index>1:
        ...
        worktime = sy_dic.get((lst[0],lst[1]),0)+ll_dic.get((lst[0],lst[1]),0)
